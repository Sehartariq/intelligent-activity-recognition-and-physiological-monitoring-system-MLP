...
@st.cache_resource
def load_models():
    # TensorFlow MLP for Activity
    activity_model = load_model("Deep Learning Module/models/activity_classification_mlp")
    ...
    ACTIVITY_FEATURES = joblib.load("Deep Learning Module/models/activity_features.pkl")

    # Physio MLP (scikit-learn) models
    ...
    return (activity_model, activity_scaler, label_encoder, ACTIVITY_FEATURES,
            physio_model, physio_scaler_X, physio_scaler_y, PHYSIO_FEATURES)
...
def predict_health(file):
    ...
    activity_pred_idx = np.argmax(activity_model.predict(X_act_scaled), axis=1)
    ...
    for i in range(len(feat_df)):
        ...
        # Convert predicted R ratio to SpO₂
        R_ratio = y[0, 1]
        spo2 = 110 - 25 * R_ratio
...
if uploaded_file:
    with st.spinner("Processing..."):
        (final_pred, window_df, ppg_green, win_samples,
         activity_pred_labels, window_hr, window_spo2) = predict_health(uploaded_file)
